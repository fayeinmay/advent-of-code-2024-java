...
public class Day5 {
    ...
    private static long puzzleOne() {
        long total = 0;
        String[] books = UPDATES.split("\n");
        String[] rules = RULES.split("\n");

        for (String book : books) {
            List<Integer> pages = Arrays.stream(book.split(",")).mapToInt(Integer::parseInt).boxed().toList();

            // If correctly ordered according to rules, add middle number to total
            if (isRulesMetForPages(pages, rules)) {
    ...
    private static long puzzleTwo() {
        ...
        Map<Integer, List<Integer>> rulesPrecedenceMap = new HashMap<>();
        for (String rule : rules) {
            String[] ruleSplit = rule.split("\\|");
            int before = Integer.parseInt(ruleSplit[0]);
            int after = Integer.parseInt(ruleSplit[1]);

            rulesPrecedenceMap.putIfAbsent(before, new ArrayList<>());
            rulesPrecedenceMap.get(before).add(after);
        }

        for (String book : books) {
            List<Integer> pages = Arrays.stream(book.split(",")).mapToInt(Integer::parseInt).boxed().collect(Collectors.toList());

            // If incorrectly ordered according to rules, order and then add middle number to total
            if (!isRulesMetForPages(pages, rules)) {
                pages.sort((a, b) -> {
                    List<Integer> aRule = rulesPrecedenceMap.get(a);
                    if (aRule != null) {
                        Optional<Integer> bRule = aRule.stream().filter(c -> Objects.equals(c, b)).findFirst();
                        if (bRule.isPresent()) {
                            return -1;
                        } else {
                            return 1;
    ...
    private static boolean isRulesMetForPages(List<Integer> pages, String[] rules) {
        ...
        for (String rule : rules) {
            ...
            int indexOfBefore = pages.indexOf(before);
            int indexOfAfter = pages.indexOf(after);
