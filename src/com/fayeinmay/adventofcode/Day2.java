...
public class Day2 {
    ...
    public static String getSolution() {
        long result;
        String[] rows = REPORTS.split("\n");

        result = puzzleOne(rows);
        result = puzzleTwo(rows);

        return Long.toString(result);
    }

    private static long puzzleOne(String[] rows) {
        long safeReports = 0;

        for (String row : rows) {
            List<Integer> numbers = new ArrayList<>(Arrays.stream(row.trim().split("\\s+"))
                    .map(Integer::valueOf)
                    .toList());

            boolean isAscending = numbers.stream().sorted()
                    .toList().equals(numbers);
            boolean isDescending = numbers.stream().sorted(Comparator.reverseOrder()).toList()
                    .equals(numbers);
    ...
    public static long puzzleTwo(String[] rows) {
        long safeReports = 0;
        ...
        for (String row : rows) {
            ...
            boolean isSafe = checkSafeReport(numbers);

            if (!isSafe) {
                isSafe = tryRemovingOneElement(numbers);
            }
    ...
    private static boolean checkSafeReport(List<Integer> numbers) {
    ...
    private static boolean tryRemovingOneElement(List<Integer> numbers) {
        for (int i = 0; i < numbers.size(); i++) {
            List<Integer> modifiedList = new ArrayList<>(numbers);
            modifiedList.remove(i);

            if (checkSafeReport(modifiedList)) {
                return true;
            }
        }
    ...
    private static long puzzleTwoFirstTry(String[] rows) {
        long safeReports = 0;
        ...
        for (String row : rows) {
            ...
            boolean problemDampenerActive = true;
            ...
            for (int i = 0; i < numbers.size() - 1; i++) {
                ...
                if (diff < 0) {
                    ...
                    isDescending = true;
                } else if (problemDampenerActive) { // Ignore zero diff one time
                    problemDampenerActive = false;
                ...
                int diffAbs = Math.abs(diff);
                if (diffAbs >= 4) {
                    if (problemDampenerActive) { // Ignore diff to high one time
                        problemDampenerActive = false;
    ...
    private static final String REPORTS_DEBUG = """
